...
with col3:
    st.markdown("**🔍 Kesif Bedeli Bilgisi**")
    kesif_adet = st.number_input("Kesif Bedeli Sayısı (Adet)", min_value=0, value=0, step=1, key="ke_a")
...
if st.button("🚀 PDF Raporunu Oluştur"):
    if pdf_file_content is None:
        ...
        try:
            ...
            # "Keşif" ibaresi "Kesif Bedeli" olarak güncellendi
            kesif_satiri = ["Kesif Bedeli", f"{kesif_adet}", f"{BIRIM_KESIF:,.2f}", "-", f"{kesif_t:,.2f} TL"]
            ...
            # Satır büyüklükleri gönderdiğin koddaki ölçülerle (110, 50, 90, 80, 90) ayarlandı
